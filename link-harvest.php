...
<?php
				die();
				break;
			case 'lh_bookmarks_export':
				$links = $wpdb->get_results("
					SELECT $wpdb->ak_linkharvest.url AS url
					, $wpdb->ak_linkharvest.title AS title
					, $wpdb->posts.post_date AS date
					, YEAR($wpdb->posts.post_date) AS year
					, MONTH($wpdb->posts.post_date) AS month
					, DAYOFMONTH($wpdb->posts.post_date) AS day
					FROM $wpdb->ak_linkharvest
					LEFT JOIN $wpdb->posts
					ON $wpdb->ak_linkharvest.post_id = $wpdb->posts.ID
					ORDER BY date
				");
				$output = '
<!DOCTYPE NETSCAPE-Bookmark-file-1>
<META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=UTF-8">
<TITLE>'.__('Link Harvest Bookmarks Export', 'alexking.org').'</TITLE>
<H1>'.__('Link Harvest Bookmarks Export', 'alexking.org').'</H1>
<DL><p>
				';
				$year = '';
				$month = '';
